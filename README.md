...
模型速查区，仅作模型参数参考。GitHub 仓库只包含代码，不附带本地模型文件。

- 选 embedding 模型时，优先看“当前真实瓶颈是什么”。
- 当前项目从 `m3e-large` 切到 `bge-m3`，主要是为了缓解长检索文本被 `512 token` 截断的问题。
- 这次切换不是因为 `1024` 维“不够高”，也不是顺手解决 GPU 环境问题。
- `embedding.model_path` 的参数意义是：**本地 embedding 模型目录**。clone 仓库后，需要先自行下载模型，再把这个参数改成你自己机器上的实际模型目录。
...
2. 改 `config/settings.yaml` 里的 `embedding.model_path`，把它指向你自己的 `bge-m3` 本地模型目录。
...
2. 改 `config/settings.yaml` 里的 `embedding.model_path`，把它指向你自己的 `m3e-large` 本地模型目录。
3. 重启 MCP 或重新加载 Codex 配置
...
- `save` 不负责解析 Markdown 文档。
- AI 在调用 `save` 前，必须先把内容整理成结构化字段。
- 如果用户指定了一份现成文档，AI 应先阅读文档，再提取并补充结构化信息。
- 传给 `save` 的 `detailed_summary` 是正式入库字段，不是临时中间值。
- 当前不再接受外部传业务时间字段；`created_at` 和 `updated_at` 都由服务端自动维护。
...
- 每次 `save` 成功后，都会同步刷新时间线和向量索引；当前实现会优先只对新记录做增量 embedding，只有缓存缺失或不一致时才退回全量重建。
- 推荐使用流程：
...
- `memory_kind`
  ...
  - 记忆类型。
  - 允许值：
    - `project_record`：项目经验类记录。
    - `chat_event`：会话事件记忆，适合保存会话中的片段性、情境性、带时间顺序的事件痕迹。一般没有特殊说明，都作为一般事件类型存储。
    - `fact`：稳定事实记忆，适合保存长期稳定、跨会话可复用、可直接指导回答的事实，更适合长期偏好、稳定约束、长期状态、已确认关系共识这类会反复影响回答的内容。
- `title`
  - 必填。
  - 记录标题，用一句话概括本次记忆。
- `short_summary`
...
- `problem_background`
  ...
  - 用于保存问题以及背景、故障现象、上下文，或为什么要记录这条记忆。
- `analysis`
  - `project_record` 必填，其他类型可选。
  - 用于保存原因判断、方案分析、取舍依据、排查结论，以及为什么这么做。
- `action_steps`
  - `project_record` 必填，其他类型可选。
  - 用于保存实际执行的步骤、命令、操作顺序或具体处理动作。
- `validation_result`
  - `project_record` 必填，其他类型可选。
  - 用于保存测试、验证、复现是否消失、验收结果等验证结论。
- `tags`
  - 可选传入，但最终存储必有。
  - 推荐由 AI 先生成标签数组。
  - 如果缺失，服务端会做基础兜底补全。
...
- 当前实现会优先只重算被更新主记忆本体及其 `field_record` 家族的向量；只有缓存缺失或当前库状态无法安全复用时，才退回全量重建。
...
5. 准备本地 embedding 模型目录，并把 `settings.yaml` 里的 `embedding.model_path` 指向它
